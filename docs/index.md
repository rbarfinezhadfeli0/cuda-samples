# Index of `(root)/`

## Overview

This folder is part of the CUDA Samples repository structure.

This is the **root directory** of the CUDA Samples repository, containing:

- Build system files
- Common utilities and libraries
- Sample categories organized by theme
- Documentation and contribution guidelines

## Subfolders (3)

### Common/

- **Path**: `Common/`
- **Index**: [View Index](./Common/index.md)
- **Documentation**: [View Documentation](./Common/doc.md)
- **Keywords**: [View Keyword Index](./Common/sub.md)

### Samples/

- **Path**: `Samples/`
- **Index**: [View Index](./Samples/index.md)
- **Documentation**: [View Documentation](./Samples/doc.md)
- **Keywords**: [View Keyword Index](./Samples/sub.md)

### cmake/

- **Path**: `cmake/`
- **Index**: [View Index](./cmake/index.md)
- **Documentation**: [View Documentation](./cmake/doc.md)
- **Keywords**: [View Keyword Index](./cmake/sub.md)

## Files (12)

| Filename | Type | Documentation | Keywords |
|----------|------|---------------|----------|
| .clang-format | (none) | [docs](./.clang-format_docs.md) | [keywords](./.clang-format_kw.md) |
| .gitignore | (none) | [docs](./.gitignore_docs.md) | [keywords](./.gitignore_kw.md) |
| .pre-commit-config.yaml | .yaml | [docs](./.pre-commit-config.yaml_docs.md) | [keywords](./.pre-commit-config.yaml_kw.md) |
| CHANGELOG.md | .md | [docs](./CHANGELOG.md_docs.md) | [keywords](./CHANGELOG.md_kw.md) |
| CMakeLists.txt | .txt | [docs](./CMakeLists.txt_docs.md) | [keywords](./CMakeLists.txt_kw.md) |
| CONTRIBUTING.md | .md | [docs](./CONTRIBUTING.md_docs.md) | [keywords](./CONTRIBUTING.md_kw.md) |
| LICENSE | (none) | [docs](./LICENSE_docs.md) | [keywords](./LICENSE_kw.md) |
| README.md | .md | [docs](./README.md_docs.md) | [keywords](./README.md_kw.md) |
| generate_comprehensive_book.py | .py | [docs](./generate_comprehensive_book.py_docs.md) | [keywords](./generate_comprehensive_book.py_kw.md) |
| generate_docs.py | .py | [docs](./generate_docs.py_docs.md) | [keywords](./generate_docs.py_kw.md) |
| run_tests.py | .py | [docs](./run_tests.py_docs.md) | [keywords](./run_tests.py_kw.md) |
| test_args.json | .json | [docs](./test_args.json_docs.md) | [keywords](./test_args.json_kw.md) |


## Navigation

- **Global Keyword Index**: [/docs/keywords.md](../keywords.md)
- **Comprehensive Book**: [/docs/comprehensive_book.md](../comprehensive_book.md)
- **Repository Root**: [/docs/index.md](../index.md)

---

*Auto-generated index*
