# Subtree Keyword Index for `Samples/0_Introduction/clock_nvrtc/`

## Scope

This keyword index covers all files within `Samples/0_Introduction/clock_nvrtc/` and all its subdirectories (recursive).

- **Files Indexed**: 7
- **Unique Keywords**: 21

## Keywords A-Z


### B

**block**
- [Samples/0_Introduction/clock_nvrtc/clock.cpp](clock.cpp_docs.md)
- [Samples/0_Introduction/clock_nvrtc/clock_kernel.cu](clock_kernel.cu_docs.md)


### C

**CMakeLists**
- [Samples/0_Introduction/clock_nvrtc/CMakeLists.txt](CMakeLists.txt_docs.md)

**c_cpp_properties**
- [Samples/0_Introduction/clock_nvrtc/.vscode/c_cpp_properties.json](.vscode/c_cpp_properties.json_docs.md)

**clock**
- [Samples/0_Introduction/clock_nvrtc/clock.cpp](clock.cpp_docs.md)

**clock_kernel**
- [Samples/0_Introduction/clock_nvrtc/clock_kernel.cu](clock_kernel.cu_docs.md)

**clock_nvrtc**
- [Samples/0_Introduction/clock_nvrtc/CMakeLists.txt](CMakeLists.txt_docs.md)

**cudaBlockSize**
- [Samples/0_Introduction/clock_nvrtc/clock.cpp](clock.cpp_docs.md)

**cudaGridSize**
- [Samples/0_Introduction/clock_nvrtc/clock.cpp](clock.cpp_docs.md)


### D

**DAMAGES**
- [Samples/0_Introduction/clock_nvrtc/clock.cpp](clock.cpp_docs.md)
- [Samples/0_Introduction/clock_nvrtc/clock_kernel.cu](clock_kernel.cu_docs.md)


### E

**extensions**
- [Samples/0_Introduction/clock_nvrtc/.vscode/extensions.json](.vscode/extensions.json_docs.md)


### G

**grid**
- [Samples/0_Introduction/clock_nvrtc/clock.cpp](clock.cpp_docs.md)
- [Samples/0_Introduction/clock_nvrtc/clock_kernel.cu](clock_kernel.cu_docs.md)


### K

**kernel**
- [Samples/0_Introduction/clock_nvrtc/clock.cpp](clock.cpp_docs.md)
- [Samples/0_Introduction/clock_nvrtc/clock_kernel.cu](clock_kernel.cu_docs.md)


### L

**launch**
- [Samples/0_Introduction/clock_nvrtc/.vscode/launch.json](.vscode/launch.json_docs.md)


### M

**main**
- [Samples/0_Introduction/clock_nvrtc/clock.cpp](clock.cpp_docs.md)


### N

**NUM_BLOCKS**
- [Samples/0_Introduction/clock_nvrtc/clock.cpp](clock.cpp_docs.md)

**NUM_THREADS**
- [Samples/0_Introduction/clock_nvrtc/clock.cpp](clock.cpp_docs.md)


### R

**README**
- [Samples/0_Introduction/clock_nvrtc/README.md](README.md_docs.md)


### T

**thread**
- [Samples/0_Introduction/clock_nvrtc/clock.cpp](clock.cpp_docs.md)
- [Samples/0_Introduction/clock_nvrtc/clock_kernel.cu](clock_kernel.cu_docs.md)

**timedReduction**
- [Samples/0_Introduction/clock_nvrtc/clock_kernel.cu](clock_kernel.cu_docs.md)



## Folder-Level Navigation

### Keywords by Subfolder

This section maps major keywords to the folders where they are most prevalent.


#### .vscode/

`c_cpp_properties`, `extensions`, `launch`

#### CMakeLists.txt/

`CMakeLists`, `clock_nvrtc`

#### README.md/

`README`

#### clock.cpp/

`DAMAGES`, `NUM_BLOCKS`, `NUM_THREADS`, `block`, `clock`, `cudaBlockSize`, `cudaGridSize`, `grid`, `kernel`, `main`, `thread`

#### clock_kernel.cu/

`DAMAGES`, `__global__`, `__shared__`, `block`, `clock_kernel`, `grid`, `kernel`, `thread`, `timedReduction`


---

*Auto-generated subtree keyword index*
