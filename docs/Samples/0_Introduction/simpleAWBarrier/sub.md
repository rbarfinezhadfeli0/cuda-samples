# Subtree Keyword Index for `Samples/0_Introduction/simpleAWBarrier/`

## Scope

This keyword index covers all files within `Samples/0_Introduction/simpleAWBarrier/` and all its subdirectories (recursive).

- **Files Indexed**: 6
- **Unique Keywords**: 26

## Keywords A-Z


### B

**block**
- [Samples/0_Introduction/simpleAWBarrier/simpleAWBarrier.cu](simpleAWBarrier.cu_docs.md)


### C

**CMakeLists**
- [Samples/0_Introduction/simpleAWBarrier/CMakeLists.txt](CMakeLists.txt_docs.md)

**c_cpp_properties**
- [Samples/0_Introduction/simpleAWBarrier/.vscode/c_cpp_properties.json](.vscode/c_cpp_properties.json_docs.md)

**cudaFree**
- [Samples/0_Introduction/simpleAWBarrier/simpleAWBarrier.cu](simpleAWBarrier.cu_docs.md)

**cudaMalloc**
- [Samples/0_Introduction/simpleAWBarrier/simpleAWBarrier.cu](simpleAWBarrier.cu_docs.md)

**cudaMemcpy**
- [Samples/0_Introduction/simpleAWBarrier/simpleAWBarrier.cu](simpleAWBarrier.cu_docs.md)


### D

**DAMAGES**
- [Samples/0_Introduction/simpleAWBarrier/simpleAWBarrier.cu](simpleAWBarrier.cu_docs.md)

**dimGrid**
- [Samples/0_Introduction/simpleAWBarrier/simpleAWBarrier.cu](simpleAWBarrier.cu_docs.md)


### E

**extensions**
- [Samples/0_Introduction/simpleAWBarrier/.vscode/extensions.json](.vscode/extensions.json_docs.md)


### F

**for**
- [Samples/0_Introduction/simpleAWBarrier/simpleAWBarrier.cu](simpleAWBarrier.cu_docs.md)


### G

**GPU**
- [Samples/0_Introduction/simpleAWBarrier/simpleAWBarrier.cu](simpleAWBarrier.cu_docs.md)

**grid**
- [Samples/0_Introduction/simpleAWBarrier/simpleAWBarrier.cu](simpleAWBarrier.cu_docs.md)


### K

**kernel**
- [Samples/0_Introduction/simpleAWBarrier/simpleAWBarrier.cu](simpleAWBarrier.cu_docs.md)


### L

**launch**
- [Samples/0_Introduction/simpleAWBarrier/.vscode/launch.json](.vscode/launch.json_docs.md)


### M

**main**
- [Samples/0_Introduction/simpleAWBarrier/simpleAWBarrier.cu](simpleAWBarrier.cu_docs.md)


### N

**normVecByDotProductAWBarrier**
- [Samples/0_Introduction/simpleAWBarrier/simpleAWBarrier.cu](simpleAWBarrier.cu_docs.md)


### R

**README**
- [Samples/0_Introduction/simpleAWBarrier/README.md](README.md_docs.md)

**reduceBlockData**
- [Samples/0_Introduction/simpleAWBarrier/simpleAWBarrier.cu](simpleAWBarrier.cu_docs.md)

**runNormVecByDotProductAWBarrier**
- [Samples/0_Introduction/simpleAWBarrier/simpleAWBarrier.cu](simpleAWBarrier.cu_docs.md)


### S

**simpleAWBarrier**
- [Samples/0_Introduction/simpleAWBarrier/CMakeLists.txt](CMakeLists.txt_docs.md)
- [Samples/0_Introduction/simpleAWBarrier/simpleAWBarrier.cu](simpleAWBarrier.cu_docs.md)


### T

**thread**
- [Samples/0_Introduction/simpleAWBarrier/simpleAWBarrier.cu](simpleAWBarrier.cu_docs.md)


### V

**Volta**
- [Samples/0_Introduction/simpleAWBarrier/simpleAWBarrier.cu](simpleAWBarrier.cu_docs.md)


### W

**warp**
- [Samples/0_Introduction/simpleAWBarrier/simpleAWBarrier.cu](simpleAWBarrier.cu_docs.md)



## Folder-Level Navigation

### Keywords by Subfolder

This section maps major keywords to the folders where they are most prevalent.


#### .vscode/

`c_cpp_properties`, `extensions`, `launch`

#### CMakeLists.txt/

`CMakeLists`, `simpleAWBarrier`

#### README.md/

`README`

#### simpleAWBarrier.cu/

`DAMAGES`, `GPU`, `Volta`, `__device__`, `__global__`, `__shared__`, `block`, `cudaFree`, `cudaMalloc`, `cudaMemcpy`, `dimGrid`, `for`, `grid`, `kernel`, `main`, `normVecByDotProductAWBarrier`, `reduceBlockData`, `runNormVecByDotProductAWBarrier`, `simpleAWBarrier`, `thread`, `warp`


---

*Auto-generated subtree keyword index*
