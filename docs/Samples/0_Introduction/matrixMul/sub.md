# Subtree Keyword Index for `Samples/0_Introduction/matrixMul/`

## Scope

This keyword index covers all files within `Samples/0_Introduction/matrixMul/` and all its subdirectories (recursive).

- **Files Indexed**: 6
- **Unique Keywords**: 30

## Keywords A-Z


### B

**block**
- [Samples/0_Introduction/matrixMul/matrixMul.cu](matrixMul.cu_docs.md)


### C

**CMakeLists**
- [Samples/0_Introduction/matrixMul/CMakeLists.txt](CMakeLists.txt_docs.md)

**ConstantInit**
- [Samples/0_Introduction/matrixMul/matrixMul.cu](matrixMul.cu_docs.md)

**c_cpp_properties**
- [Samples/0_Introduction/matrixMul/.vscode/c_cpp_properties.json](.vscode/c_cpp_properties.json_docs.md)

**checkCudaErrors**
- [Samples/0_Introduction/matrixMul/matrixMul.cu](matrixMul.cu_docs.md)

**cudaFree**
- [Samples/0_Introduction/matrixMul/matrixMul.cu](matrixMul.cu_docs.md)

**cudaMalloc**
- [Samples/0_Introduction/matrixMul/matrixMul.cu](matrixMul.cu_docs.md)

**cudaMemcpy**
- [Samples/0_Introduction/matrixMul/matrixMul.cu](matrixMul.cu_docs.md)


### D

**DAMAGES**
- [Samples/0_Introduction/matrixMul/matrixMul.cu](matrixMul.cu_docs.md)

**dimsA**
- [Samples/0_Introduction/matrixMul/matrixMul.cu](matrixMul.cu_docs.md)

**dimsB**
- [Samples/0_Introduction/matrixMul/matrixMul.cu](matrixMul.cu_docs.md)

**dimsC**
- [Samples/0_Introduction/matrixMul/matrixMul.cu](matrixMul.cu_docs.md)


### E

**extensions**
- [Samples/0_Introduction/matrixMul/.vscode/extensions.json](.vscode/extensions.json_docs.md)


### F

**for**
- [Samples/0_Introduction/matrixMul/matrixMul.cu](matrixMul.cu_docs.md)


### G

**grid**
- [Samples/0_Introduction/matrixMul/matrixMul.cu](matrixMul.cu_docs.md)


### K

**kernel**
- [Samples/0_Introduction/matrixMul/matrixMul.cu](matrixMul.cu_docs.md)


### L

**launch**
- [Samples/0_Introduction/matrixMul/.vscode/launch.json](.vscode/launch.json_docs.md)


### M

**MatrixMulCUDA**
- [Samples/0_Introduction/matrixMul/matrixMul.cu](matrixMul.cu_docs.md)

**MatrixMultiply**
- [Samples/0_Introduction/matrixMul/matrixMul.cu](matrixMul.cu_docs.md)

**main**
- [Samples/0_Introduction/matrixMul/matrixMul.cu](matrixMul.cu_docs.md)

**matrixMul**
- [Samples/0_Introduction/matrixMul/CMakeLists.txt](CMakeLists.txt_docs.md)
- [Samples/0_Introduction/matrixMul/matrixMul.cu](matrixMul.cu_docs.md)

**multiplication**
- [Samples/0_Introduction/matrixMul/matrixMul.cu](matrixMul.cu_docs.md)


### P

**printf**
- [Samples/0_Introduction/matrixMul/matrixMul.cu](matrixMul.cu_docs.md)


### R

**README**
- [Samples/0_Introduction/matrixMul/README.md](README.md_docs.md)


### S

**Supercomputing**
- [Samples/0_Introduction/matrixMul/matrixMul.cu](matrixMul.cu_docs.md)


### T

**thread**
- [Samples/0_Introduction/matrixMul/matrixMul.cu](matrixMul.cu_docs.md)

**threads**
- [Samples/0_Introduction/matrixMul/matrixMul.cu](matrixMul.cu_docs.md)



## Folder-Level Navigation

### Keywords by Subfolder

This section maps major keywords to the folders where they are most prevalent.


#### .vscode/

`c_cpp_properties`, `extensions`, `launch`

#### CMakeLists.txt/

`CMakeLists`, `matrixMul`

#### README.md/

`README`

#### matrixMul.cu/

`ConstantInit`, `DAMAGES`, `MatrixMulCUDA`, `MatrixMultiply`, `Supercomputing`, `__global__`, `__shared__`, `__syncthreads`, `block`, `checkCudaErrors`, `cudaFree`, `cudaMalloc`, `cudaMemcpy`, `dimsA`, `dimsB`, `dimsC`, `for`, `grid`, `kernel`, `main`, `matrixMul`, `multiplication`, `printf`, `thread`, `threads`


---

*Auto-generated subtree keyword index*
