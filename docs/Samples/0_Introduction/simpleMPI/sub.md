# Subtree Keyword Index for `Samples/0_Introduction/simpleMPI/`

## Scope

This keyword index covers all files within `Samples/0_Introduction/simpleMPI/` and all its subdirectories (recursive).

- **Files Indexed**: 8
- **Unique Keywords**: 25

## Keywords A-Z


### B

**block**
- [Samples/0_Introduction/simpleMPI/simpleMPI.cpp](simpleMPI.cpp_docs.md)
- [Samples/0_Introduction/simpleMPI/simpleMPI.cu](simpleMPI.cu_docs.md)
- [Samples/0_Introduction/simpleMPI/simpleMPI.h](simpleMPI.h_docs.md)


### C

**CMakeLists**
- [Samples/0_Introduction/simpleMPI/CMakeLists.txt](CMakeLists.txt_docs.md)

**CUDA_CHECK**
- [Samples/0_Introduction/simpleMPI/simpleMPI.cu](simpleMPI.cu_docs.md)

**c_cpp_properties**
- [Samples/0_Introduction/simpleMPI/.vscode/c_cpp_properties.json](.vscode/c_cpp_properties.json_docs.md)

**computeGPU**
- [Samples/0_Introduction/simpleMPI/simpleMPI.cpp](simpleMPI.cpp_docs.md)
- [Samples/0_Introduction/simpleMPI/simpleMPI.cu](simpleMPI.cu_docs.md)
- [Samples/0_Introduction/simpleMPI/simpleMPI.h](simpleMPI.h_docs.md)

**cudaFree**
- [Samples/0_Introduction/simpleMPI/simpleMPI.cu](simpleMPI.cu_docs.md)

**cudaMalloc**
- [Samples/0_Introduction/simpleMPI/simpleMPI.cu](simpleMPI.cu_docs.md)

**cudaMemcpy**
- [Samples/0_Introduction/simpleMPI/simpleMPI.cu](simpleMPI.cu_docs.md)


### D

**DAMAGES**
- [Samples/0_Introduction/simpleMPI/simpleMPI.cpp](simpleMPI.cpp_docs.md)
- [Samples/0_Introduction/simpleMPI/simpleMPI.cu](simpleMPI.cu_docs.md)
- [Samples/0_Introduction/simpleMPI/simpleMPI.h](simpleMPI.h_docs.md)

**data**
- [Samples/0_Introduction/simpleMPI/simpleMPI.cu](simpleMPI.cu_docs.md)


### E

**extensions**
- [Samples/0_Introduction/simpleMPI/.vscode/extensions.json](.vscode/extensions.json_docs.md)


### G

**grid**
- [Samples/0_Introduction/simpleMPI/simpleMPI.cpp](simpleMPI.cpp_docs.md)
- [Samples/0_Introduction/simpleMPI/simpleMPI.cu](simpleMPI.cu_docs.md)
- [Samples/0_Introduction/simpleMPI/simpleMPI.h](simpleMPI.h_docs.md)


### I

**initData**
- [Samples/0_Introduction/simpleMPI/simpleMPI.cu](simpleMPI.cu_docs.md)
- [Samples/0_Introduction/simpleMPI/simpleMPI.h](simpleMPI.h_docs.md)


### K

**kernel**
- [Samples/0_Introduction/simpleMPI/simpleMPI.cu](simpleMPI.cu_docs.md)


### L

**launch**
- [Samples/0_Introduction/simpleMPI/.vscode/launch.json](.vscode/launch.json_docs.md)


### M

**MPI_CHECK**
- [Samples/0_Introduction/simpleMPI/simpleMPI.cpp](simpleMPI.cpp_docs.md)

**main**
- [Samples/0_Introduction/simpleMPI/simpleMPI.cpp](simpleMPI.cpp_docs.md)

**my_abort**
- [Samples/0_Introduction/simpleMPI/simpleMPI.cpp](simpleMPI.cpp_docs.md)
- [Samples/0_Introduction/simpleMPI/simpleMPI.h](simpleMPI.h_docs.md)


### N

**node**
- [Samples/0_Introduction/simpleMPI/simpleMPI.cpp](simpleMPI.cpp_docs.md)


### R

**README**
- [Samples/0_Introduction/simpleMPI/README.md](README.md_docs.md)


### S

**simpleMPI**
- [Samples/0_Introduction/simpleMPI/CMakeLists.txt](CMakeLists.txt_docs.md)
- [Samples/0_Introduction/simpleMPI/simpleMPI.cpp](simpleMPI.cpp_docs.md)
- [Samples/0_Introduction/simpleMPI/simpleMPI.cu](simpleMPI.cu_docs.md)
- [Samples/0_Introduction/simpleMPI/simpleMPI.h](simpleMPI.h_docs.md)

**simpleMPIKernel**
- [Samples/0_Introduction/simpleMPI/simpleMPI.cu](simpleMPI.cu_docs.md)

**sum**
- [Samples/0_Introduction/simpleMPI/simpleMPI.cu](simpleMPI.cu_docs.md)
- [Samples/0_Introduction/simpleMPI/simpleMPI.h](simpleMPI.h_docs.md)


### T

**thread**
- [Samples/0_Introduction/simpleMPI/simpleMPI.cu](simpleMPI.cu_docs.md)



## Folder-Level Navigation

### Keywords by Subfolder

This section maps major keywords to the folders where they are most prevalent.


#### .vscode/

`c_cpp_properties`, `extensions`, `launch`

#### CMakeLists.txt/

`CMakeLists`, `simpleMPI`

#### README.md/

`README`

#### simpleMPI.cpp/

`DAMAGES`, `MPI_CHECK`, `block`, `computeGPU`, `grid`, `main`, `my_abort`, `node`, `simpleMPI`

#### simpleMPI.cu/

`CUDA_CHECK`, `DAMAGES`, `__global__`, `block`, `computeGPU`, `cudaFree`, `cudaMalloc`, `cudaMemcpy`, `data`, `grid`, `initData`, `kernel`, `simpleMPI`, `simpleMPIKernel`, `sum`, `thread`

#### simpleMPI.h/

`DAMAGES`, `block`, `computeGPU`, `grid`, `initData`, `my_abort`, `simpleMPI`, `sum`


---

*Auto-generated subtree keyword index*
