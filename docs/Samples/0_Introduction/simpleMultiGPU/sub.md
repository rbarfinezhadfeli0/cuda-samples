# Subtree Keyword Index for `Samples/0_Introduction/simpleMultiGPU/`

## Scope

This keyword index covers all files within `Samples/0_Introduction/simpleMultiGPU/` and all its subdirectories (recursive).

- **Files Indexed**: 7
- **Unique Keywords**: 27

## Keywords A-Z


### B

**block**
- [Samples/0_Introduction/simpleMultiGPU/simpleMultiGPU.cu](simpleMultiGPU.cu_docs.md)


### C

**CMakeLists**
- [Samples/0_Introduction/simpleMultiGPU/CMakeLists.txt](CMakeLists.txt_docs.md)

**c_cpp_properties**
- [Samples/0_Introduction/simpleMultiGPU/.vscode/c_cpp_properties.json](.vscode/c_cpp_properties.json_docs.md)

**checkCudaErrors**
- [Samples/0_Introduction/simpleMultiGPU/simpleMultiGPU.cu](simpleMultiGPU.cu_docs.md)

**cudaFree**
- [Samples/0_Introduction/simpleMultiGPU/simpleMultiGPU.cu](simpleMultiGPU.cu_docs.md)

**cudaMalloc**
- [Samples/0_Introduction/simpleMultiGPU/simpleMultiGPU.cu](simpleMultiGPU.cu_docs.md)

**cudaMemcpy**
- [Samples/0_Introduction/simpleMultiGPU/simpleMultiGPU.cu](simpleMultiGPU.cu_docs.md)

**cudaStreamSynchronize**
- [Samples/0_Introduction/simpleMultiGPU/simpleMultiGPU.cu](simpleMultiGPU.cu_docs.md)


### D

**DAMAGES**
- [Samples/0_Introduction/simpleMultiGPU/simpleMultiGPU.cu](simpleMultiGPU.cu_docs.md)
- [Samples/0_Introduction/simpleMultiGPU/simpleMultiGPU.h](simpleMultiGPU.h_docs.md)


### E

**extensions**
- [Samples/0_Introduction/simpleMultiGPU/.vscode/extensions.json](.vscode/extensions.json_docs.md)


### F

**for**
- [Samples/0_Introduction/simpleMultiGPU/simpleMultiGPU.cu](simpleMultiGPU.cu_docs.md)


### G

**GPU**
- [Samples/0_Introduction/simpleMultiGPU/simpleMultiGPU.cu](simpleMultiGPU.cu_docs.md)

**grid**
- [Samples/0_Introduction/simpleMultiGPU/simpleMultiGPU.cu](simpleMultiGPU.cu_docs.md)


### K

**kernel**
- [Samples/0_Introduction/simpleMultiGPU/simpleMultiGPU.cu](simpleMultiGPU.cu_docs.md)


### L

**launch**
- [Samples/0_Introduction/simpleMultiGPU/.vscode/launch.json](.vscode/launch.json_docs.md)

**launch_reduceKernel**
- [Samples/0_Introduction/simpleMultiGPU/simpleMultiGPU.h](simpleMultiGPU.h_docs.md)


### M

**MAX**
- [Samples/0_Introduction/simpleMultiGPU/simpleMultiGPU.cu](simpleMultiGPU.cu_docs.md)

**main**
- [Samples/0_Introduction/simpleMultiGPU/simpleMultiGPU.cu](simpleMultiGPU.cu_docs.md)


### P

**printf**
- [Samples/0_Introduction/simpleMultiGPU/simpleMultiGPU.cu](simpleMultiGPU.cu_docs.md)


### R

**README**
- [Samples/0_Introduction/simpleMultiGPU/README.md](README.md_docs.md)

**reduceKernel**
- [Samples/0_Introduction/simpleMultiGPU/simpleMultiGPU.cu](simpleMultiGPU.cu_docs.md)


### S

**SIMPLEMULTIGPU_H**
- [Samples/0_Introduction/simpleMultiGPU/simpleMultiGPU.h](simpleMultiGPU.h_docs.md)

**sdkStartTimer**
- [Samples/0_Introduction/simpleMultiGPU/simpleMultiGPU.cu](simpleMultiGPU.cu_docs.md)

**simpleMultiGPU**
- [Samples/0_Introduction/simpleMultiGPU/CMakeLists.txt](CMakeLists.txt_docs.md)
- [Samples/0_Introduction/simpleMultiGPU/simpleMultiGPU.cu](simpleMultiGPU.cu_docs.md)
- [Samples/0_Introduction/simpleMultiGPU/simpleMultiGPU.h](simpleMultiGPU.h_docs.md)


### T

**techniques**
- [Samples/0_Introduction/simpleMultiGPU/simpleMultiGPU.cu](simpleMultiGPU.cu_docs.md)

**thread**
- [Samples/0_Introduction/simpleMultiGPU/simpleMultiGPU.cu](simpleMultiGPU.cu_docs.md)



## Folder-Level Navigation

### Keywords by Subfolder

This section maps major keywords to the folders where they are most prevalent.


#### .vscode/

`c_cpp_properties`, `extensions`, `launch`

#### CMakeLists.txt/

`CMakeLists`, `simpleMultiGPU`

#### README.md/

`README`

#### simpleMultiGPU.cu/

`DAMAGES`, `GPU`, `MAX`, `__global__`, `block`, `checkCudaErrors`, `cudaFree`, `cudaMalloc`, `cudaMemcpy`, `cudaStreamSynchronize`, `for`, `grid`, `kernel`, `main`, `printf`, `reduceKernel`, `sdkStartTimer`, `simpleMultiGPU`, `techniques`, `thread`

#### simpleMultiGPU.h/

`DAMAGES`, `SIMPLEMULTIGPU_H`, `launch_reduceKernel`, `simpleMultiGPU`


---

*Auto-generated subtree keyword index*
