# Subtree Keyword Index for `Samples/0_Introduction/simpleIPC/`

## Scope

This keyword index covers all files within `Samples/0_Introduction/simpleIPC/` and all its subdirectories (recursive).

- **Files Indexed**: 6
- **Unique Keywords**: 31

## Keywords A-Z


### B

**barrierWait**
- [Samples/0_Introduction/simpleIPC/simpleIPC.cu](simpleIPC.cu_docs.md)

**block**
- [Samples/0_Introduction/simpleIPC/simpleIPC.cu](simpleIPC.cu_docs.md)


### C

**CMakeLists**
- [Samples/0_Introduction/simpleIPC/CMakeLists.txt](CMakeLists.txt_docs.md)

**c_cpp_properties**
- [Samples/0_Introduction/simpleIPC/.vscode/c_cpp_properties.json](.vscode/c_cpp_properties.json_docs.md)

**checkCudaErrors**
- [Samples/0_Introduction/simpleIPC/simpleIPC.cu](simpleIPC.cu_docs.md)

**childProcess**
- [Samples/0_Introduction/simpleIPC/simpleIPC.cu](simpleIPC.cu_docs.md)

**cpu_atomic_add32**
- [Samples/0_Introduction/simpleIPC/simpleIPC.cu](simpleIPC.cu_docs.md)

**cudaFree**
- [Samples/0_Introduction/simpleIPC/simpleIPC.cu](simpleIPC.cu_docs.md)

**cudaMalloc**
- [Samples/0_Introduction/simpleIPC/simpleIPC.cu](simpleIPC.cu_docs.md)

**cudaMemcpy**
- [Samples/0_Introduction/simpleIPC/simpleIPC.cu](simpleIPC.cu_docs.md)


### D

**DAMAGES**
- [Samples/0_Introduction/simpleIPC/simpleIPC.cu](simpleIPC.cu_docs.md)

**DATA_SIZE**
- [Samples/0_Introduction/simpleIPC/simpleIPC.cu](simpleIPC.cu_docs.md)

**defined**
- [Samples/0_Introduction/simpleIPC/simpleIPC.cu](simpleIPC.cu_docs.md)


### E

**exec**
- [Samples/0_Introduction/simpleIPC/simpleIPC.cu](simpleIPC.cu_docs.md)

**extensions**
- [Samples/0_Introduction/simpleIPC/.vscode/extensions.json](.vscode/extensions.json_docs.md)


### F

**for**
- [Samples/0_Introduction/simpleIPC/simpleIPC.cu](simpleIPC.cu_docs.md)

**fork**
- [Samples/0_Introduction/simpleIPC/simpleIPC.cu](simpleIPC.cu_docs.md)


### G

**grid**
- [Samples/0_Introduction/simpleIPC/simpleIPC.cu](simpleIPC.cu_docs.md)


### K

**kernel**
- [Samples/0_Introduction/simpleIPC/simpleIPC.cu](simpleIPC.cu_docs.md)


### L

**launch**
- [Samples/0_Introduction/simpleIPC/.vscode/launch.json](.vscode/launch.json_docs.md)


### M

**MAX_DEVICES**
- [Samples/0_Introduction/simpleIPC/simpleIPC.cu](simpleIPC.cu_docs.md)

**main**
- [Samples/0_Introduction/simpleIPC/simpleIPC.cu](simpleIPC.cu_docs.md)


### P

**parentProcess**
- [Samples/0_Introduction/simpleIPC/simpleIPC.cu](simpleIPC.cu_docs.md)

**process**
- [Samples/0_Introduction/simpleIPC/simpleIPC.cu](simpleIPC.cu_docs.md)


### R

**README**
- [Samples/0_Introduction/simpleIPC/README.md](README.md_docs.md)


### S

**shmStruct_st**
- [Samples/0_Introduction/simpleIPC/simpleIPC.cu](simpleIPC.cu_docs.md)

**simpleIPC**
- [Samples/0_Introduction/simpleIPC/CMakeLists.txt](CMakeLists.txt_docs.md)
- [Samples/0_Introduction/simpleIPC/simpleIPC.cu](simpleIPC.cu_docs.md)

**simpleKernel**
- [Samples/0_Introduction/simpleIPC/simpleIPC.cu](simpleIPC.cu_docs.md)

**strcat**
- [Samples/0_Introduction/simpleIPC/simpleIPC.cu](simpleIPC.cu_docs.md)


### T

**thread**
- [Samples/0_Introduction/simpleIPC/simpleIPC.cu](simpleIPC.cu_docs.md)



## Folder-Level Navigation

### Keywords by Subfolder

This section maps major keywords to the folders where they are most prevalent.


#### .vscode/

`c_cpp_properties`, `extensions`, `launch`

#### CMakeLists.txt/

`CMakeLists`, `simpleIPC`

#### README.md/

`README`

#### simpleIPC.cu/

`DAMAGES`, `DATA_SIZE`, `MAX_DEVICES`, `__global__`, `barrierWait`, `block`, `checkCudaErrors`, `childProcess`, `cpu_atomic_add32`, `cudaFree`, `cudaMalloc`, `cudaMemcpy`, `defined`, `exec`, `for`, `fork`, `grid`, `kernel`, `main`, `parentProcess`, `process`, `shmStruct_st`, `simpleIPC`, `simpleKernel`, `strcat`, `thread`


---

*Auto-generated subtree keyword index*
