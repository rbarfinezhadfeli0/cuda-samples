# Subtree Keyword Index for `Samples/0_Introduction/simpleAssert/`

## Scope

This keyword index covers all files within `Samples/0_Introduction/simpleAssert/` and all its subdirectories (recursive).

- **Files Indexed**: 6
- **Unique Keywords**: 20

## Keywords A-Z


### B

**block**
- [Samples/0_Introduction/simpleAssert/simpleAssert.cu](simpleAssert.cu_docs.md)


### C

**CMakeLists**
- [Samples/0_Introduction/simpleAssert/CMakeLists.txt](CMakeLists.txt_docs.md)

**c_cpp_properties**
- [Samples/0_Introduction/simpleAssert/.vscode/c_cpp_properties.json](.vscode/c_cpp_properties.json_docs.md)


### D

**DAMAGES**
- [Samples/0_Introduction/simpleAssert/simpleAssert.cu](simpleAssert.cu_docs.md)

**dimBlock**
- [Samples/0_Introduction/simpleAssert/simpleAssert.cu](simpleAssert.cu_docs.md)

**dimGrid**
- [Samples/0_Introduction/simpleAssert/simpleAssert.cu](simpleAssert.cu_docs.md)


### E

**extensions**
- [Samples/0_Introduction/simpleAssert/.vscode/extensions.json](.vscode/extensions.json_docs.md)


### F

**findCudaDevice**
- [Samples/0_Introduction/simpleAssert/simpleAssert.cu](simpleAssert.cu_docs.md)


### G

**grid**
- [Samples/0_Introduction/simpleAssert/simpleAssert.cu](simpleAssert.cu_docs.md)


### K

**kernel**
- [Samples/0_Introduction/simpleAssert/simpleAssert.cu](simpleAssert.cu_docs.md)


### L

**launch**
- [Samples/0_Introduction/simpleAssert/.vscode/launch.json](.vscode/launch.json_docs.md)


### M

**main**
- [Samples/0_Introduction/simpleAssert/simpleAssert.cu](simpleAssert.cu_docs.md)


### N

**NOMINMAX**
- [Samples/0_Introduction/simpleAssert/simpleAssert.cu](simpleAssert.cu_docs.md)


### R

**README**
- [Samples/0_Introduction/simpleAssert/README.md](README.md_docs.md)

**runTest**
- [Samples/0_Introduction/simpleAssert/simpleAssert.cu](simpleAssert.cu_docs.md)


### S

**simpleAssert**
- [Samples/0_Introduction/simpleAssert/CMakeLists.txt](CMakeLists.txt_docs.md)
- [Samples/0_Introduction/simpleAssert/simpleAssert.cu](simpleAssert.cu_docs.md)


### T

**testKernel**
- [Samples/0_Introduction/simpleAssert/simpleAssert.cu](simpleAssert.cu_docs.md)

**thread**
- [Samples/0_Introduction/simpleAssert/simpleAssert.cu](simpleAssert.cu_docs.md)


### W

**WINDOWS_LEAN_AND_MEAN**
- [Samples/0_Introduction/simpleAssert/simpleAssert.cu](simpleAssert.cu_docs.md)



## Folder-Level Navigation

### Keywords by Subfolder

This section maps major keywords to the folders where they are most prevalent.


#### .vscode/

`c_cpp_properties`, `extensions`, `launch`

#### CMakeLists.txt/

`CMakeLists`, `simpleAssert`

#### README.md/

`README`

#### simpleAssert.cu/

`DAMAGES`, `NOMINMAX`, `WINDOWS_LEAN_AND_MEAN`, `__global__`, `block`, `dimBlock`, `dimGrid`, `findCudaDevice`, `grid`, `kernel`, `main`, `runTest`, `simpleAssert`, `testKernel`, `thread`


---

*Auto-generated subtree keyword index*
