# Subtree Keyword Index for `Samples/0_Introduction/simpleZeroCopy/`

## Scope

This keyword index covers all files within `Samples/0_Introduction/simpleZeroCopy/` and all its subdirectories (recursive).

- **Files Indexed**: 6
- **Unique Keywords**: 24

## Keywords A-Z


### A

**ALIGN_UP**
- [Samples/0_Introduction/simpleZeroCopy/simpleZeroCopy.cu](simpleZeroCopy.cu_docs.md)

**Allocated**
- [Samples/0_Introduction/simpleZeroCopy/simpleZeroCopy.cu](simpleZeroCopy.cu_docs.md)


### B

**block**
- [Samples/0_Introduction/simpleZeroCopy/simpleZeroCopy.cu](simpleZeroCopy.cu_docs.md)


### C

**CMakeLists**
- [Samples/0_Introduction/simpleZeroCopy/CMakeLists.txt](CMakeLists.txt_docs.md)

**c_cpp_properties**
- [Samples/0_Introduction/simpleZeroCopy/.vscode/c_cpp_properties.json](.vscode/c_cpp_properties.json_docs.md)

**checkCudaErrors**
- [Samples/0_Introduction/simpleZeroCopy/simpleZeroCopy.cu](simpleZeroCopy.cu_docs.md)

**cudaFree**
- [Samples/0_Introduction/simpleZeroCopy/simpleZeroCopy.cu](simpleZeroCopy.cu_docs.md)


### D

**DAMAGES**
- [Samples/0_Introduction/simpleZeroCopy/simpleZeroCopy.cu](simpleZeroCopy.cu_docs.md)

**defined**
- [Samples/0_Introduction/simpleZeroCopy/simpleZeroCopy.cu](simpleZeroCopy.cu_docs.md)


### E

**extensions**
- [Samples/0_Introduction/simpleZeroCopy/.vscode/extensions.json](.vscode/extensions.json_docs.md)


### F

**fprintf**
- [Samples/0_Introduction/simpleZeroCopy/simpleZeroCopy.cu](simpleZeroCopy.cu_docs.md)


### G

**grid**
- [Samples/0_Introduction/simpleZeroCopy/simpleZeroCopy.cu](simpleZeroCopy.cu_docs.md)


### K

**kernel**
- [Samples/0_Introduction/simpleZeroCopy/simpleZeroCopy.cu](simpleZeroCopy.cu_docs.md)


### L

**launch**
- [Samples/0_Introduction/simpleZeroCopy/.vscode/launch.json](.vscode/launch.json_docs.md)


### M

**MAX**
- [Samples/0_Introduction/simpleZeroCopy/simpleZeroCopy.cu](simpleZeroCopy.cu_docs.md)

**MEMORY_ALIGNMENT**
- [Samples/0_Introduction/simpleZeroCopy/simpleZeroCopy.cu](simpleZeroCopy.cu_docs.md)

**Memory**
- [Samples/0_Introduction/simpleZeroCopy/simpleZeroCopy.cu](simpleZeroCopy.cu_docs.md)

**main**
- [Samples/0_Introduction/simpleZeroCopy/simpleZeroCopy.cu](simpleZeroCopy.cu_docs.md)

**malloc**
- [Samples/0_Introduction/simpleZeroCopy/simpleZeroCopy.cu](simpleZeroCopy.cu_docs.md)


### R

**README**
- [Samples/0_Introduction/simpleZeroCopy/README.md](README.md_docs.md)


### S

**simpleZeroCopy**
- [Samples/0_Introduction/simpleZeroCopy/CMakeLists.txt](CMakeLists.txt_docs.md)
- [Samples/0_Introduction/simpleZeroCopy/simpleZeroCopy.cu](simpleZeroCopy.cu_docs.md)


### T

**thread**
- [Samples/0_Introduction/simpleZeroCopy/simpleZeroCopy.cu](simpleZeroCopy.cu_docs.md)


### V

**vectorAddGPU**
- [Samples/0_Introduction/simpleZeroCopy/simpleZeroCopy.cu](simpleZeroCopy.cu_docs.md)



## Folder-Level Navigation

### Keywords by Subfolder

This section maps major keywords to the folders where they are most prevalent.


#### .vscode/

`c_cpp_properties`, `extensions`, `launch`

#### CMakeLists.txt/

`CMakeLists`, `simpleZeroCopy`

#### README.md/

`README`

#### simpleZeroCopy.cu/

`ALIGN_UP`, `Allocated`, `DAMAGES`, `MAX`, `MEMORY_ALIGNMENT`, `Memory`, `__global__`, `block`, `checkCudaErrors`, `cudaFree`, `defined`, `fprintf`, `grid`, `kernel`, `main`, `malloc`, `simpleZeroCopy`, `thread`, `vectorAddGPU`


---

*Auto-generated subtree keyword index*
