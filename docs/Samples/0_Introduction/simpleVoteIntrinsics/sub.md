# Subtree Keyword Index for `Samples/0_Introduction/simpleVoteIntrinsics/`

## Scope

This keyword index covers all files within `Samples/0_Introduction/simpleVoteIntrinsics/` and all its subdirectories (recursive).

- **Files Indexed**: 7
- **Unique Keywords**: 37

## Keywords A-Z


### C

**CMakeLists**
- [Samples/0_Introduction/simpleVoteIntrinsics/CMakeLists.txt](CMakeLists.txt_docs.md)

**c_cpp_properties**
- [Samples/0_Introduction/simpleVoteIntrinsics/.vscode/c_cpp_properties.json](.vscode/c_cpp_properties.json_docs.md)

**checkCudaErrors**
- [Samples/0_Introduction/simpleVoteIntrinsics/simpleVoteIntrinsics.cu](simpleVoteIntrinsics.cu_docs.md)

**checkErrors1**
- [Samples/0_Introduction/simpleVoteIntrinsics/simpleVoteIntrinsics.cu](simpleVoteIntrinsics.cu_docs.md)

**checkErrors2**
- [Samples/0_Introduction/simpleVoteIntrinsics/simpleVoteIntrinsics.cu](simpleVoteIntrinsics.cu_docs.md)

**checkResultsVoteAllKernel2**
- [Samples/0_Introduction/simpleVoteIntrinsics/simpleVoteIntrinsics.cu](simpleVoteIntrinsics.cu_docs.md)

**checkResultsVoteAnyKernel1**
- [Samples/0_Introduction/simpleVoteIntrinsics/simpleVoteIntrinsics.cu](simpleVoteIntrinsics.cu_docs.md)

**checkResultsVoteAnyKernel3**
- [Samples/0_Introduction/simpleVoteIntrinsics/simpleVoteIntrinsics.cu](simpleVoteIntrinsics.cu_docs.md)

**cudaFree**
- [Samples/0_Introduction/simpleVoteIntrinsics/simpleVoteIntrinsics.cu](simpleVoteIntrinsics.cu_docs.md)

**cudaMalloc**
- [Samples/0_Introduction/simpleVoteIntrinsics/simpleVoteIntrinsics.cu](simpleVoteIntrinsics.cu_docs.md)

**cudaMemcpy**
- [Samples/0_Introduction/simpleVoteIntrinsics/simpleVoteIntrinsics.cu](simpleVoteIntrinsics.cu_docs.md)


### D

**DAMAGES**
- [Samples/0_Introduction/simpleVoteIntrinsics/simpleVoteIntrinsics.cu](simpleVoteIntrinsics.cu_docs.md)
- [Samples/0_Introduction/simpleVoteIntrinsics/simpleVote_kernel.cuh](simpleVote_kernel.cuh_docs.md)


### E

**extensions**
- [Samples/0_Introduction/simpleVoteIntrinsics/.vscode/extensions.json](.vscode/extensions.json_docs.md)


### F

**Functions**
- [Samples/0_Introduction/simpleVoteIntrinsics/simpleVote_kernel.cuh](simpleVote_kernel.cuh_docs.md)

**free**
- [Samples/0_Introduction/simpleVoteIntrinsics/simpleVoteIntrinsics.cu](simpleVoteIntrinsics.cu_docs.md)


### G

**genVoteTestPattern**
- [Samples/0_Introduction/simpleVoteIntrinsics/simpleVoteIntrinsics.cu](simpleVoteIntrinsics.cu_docs.md)

**grid**
- [Samples/0_Introduction/simpleVoteIntrinsics/simpleVoteIntrinsics.cu](simpleVoteIntrinsics.cu_docs.md)

**gridBlock**
- [Samples/0_Introduction/simpleVoteIntrinsics/simpleVoteIntrinsics.cu](simpleVoteIntrinsics.cu_docs.md)


### K

**kernel**
- [Samples/0_Introduction/simpleVoteIntrinsics/simpleVoteIntrinsics.cu](simpleVoteIntrinsics.cu_docs.md)
- [Samples/0_Introduction/simpleVoteIntrinsics/simpleVote_kernel.cuh](simpleVote_kernel.cuh_docs.md)


### L

**launch**
- [Samples/0_Introduction/simpleVoteIntrinsics/.vscode/launch.json](.vscode/launch.json_docs.md)


### M

**MAX**
- [Samples/0_Introduction/simpleVoteIntrinsics/simpleVoteIntrinsics.cu](simpleVoteIntrinsics.cu_docs.md)

**main**
- [Samples/0_Introduction/simpleVoteIntrinsics/simpleVoteIntrinsics.cu](simpleVoteIntrinsics.cu_docs.md)


### P

**printf**
- [Samples/0_Introduction/simpleVoteIntrinsics/simpleVoteIntrinsics.cu](simpleVoteIntrinsics.cu_docs.md)


### R

**README**
- [Samples/0_Introduction/simpleVoteIntrinsics/README.md](README.md_docs.md)


### S

**SIMPLEVOTE_KERNEL_CU**
- [Samples/0_Introduction/simpleVoteIntrinsics/simpleVote_kernel.cuh](simpleVote_kernel.cuh_docs.md)

**simpleVoteIntrinsics**
- [Samples/0_Introduction/simpleVoteIntrinsics/CMakeLists.txt](CMakeLists.txt_docs.md)
- [Samples/0_Introduction/simpleVoteIntrinsics/simpleVoteIntrinsics.cu](simpleVoteIntrinsics.cu_docs.md)

**simpleVote_kernel**
- [Samples/0_Introduction/simpleVoteIntrinsics/simpleVote_kernel.cuh](simpleVote_kernel.cuh_docs.md)


### T

**thread**
- [Samples/0_Introduction/simpleVoteIntrinsics/simpleVoteIntrinsics.cu](simpleVoteIntrinsics.cu_docs.md)
- [Samples/0_Introduction/simpleVoteIntrinsics/simpleVote_kernel.cuh](simpleVote_kernel.cuh_docs.md)

**threadBlock**
- [Samples/0_Introduction/simpleVoteIntrinsics/simpleVoteIntrinsics.cu](simpleVoteIntrinsics.cu_docs.md)

**threads**
- [Samples/0_Introduction/simpleVoteIntrinsics/simpleVote_kernel.cuh](simpleVote_kernel.cuh_docs.md)


### V

**VOTE_DATA_GROUP**
- [Samples/0_Introduction/simpleVoteIntrinsics/simpleVoteIntrinsics.cu](simpleVoteIntrinsics.cu_docs.md)

**VoteAllKernel2**
- [Samples/0_Introduction/simpleVoteIntrinsics/simpleVote_kernel.cuh](simpleVote_kernel.cuh_docs.md)

**VoteAnyKernel1**
- [Samples/0_Introduction/simpleVoteIntrinsics/simpleVote_kernel.cuh](simpleVote_kernel.cuh_docs.md)

**VoteAnyKernel3**
- [Samples/0_Introduction/simpleVoteIntrinsics/simpleVote_kernel.cuh](simpleVote_kernel.cuh_docs.md)

**vote**
- [Samples/0_Introduction/simpleVoteIntrinsics/simpleVote_kernel.cuh](simpleVote_kernel.cuh_docs.md)


### W

**warp**
- [Samples/0_Introduction/simpleVoteIntrinsics/simpleVoteIntrinsics.cu](simpleVoteIntrinsics.cu_docs.md)
- [Samples/0_Introduction/simpleVoteIntrinsics/simpleVote_kernel.cuh](simpleVote_kernel.cuh_docs.md)



## Folder-Level Navigation

### Keywords by Subfolder

This section maps major keywords to the folders where they are most prevalent.


#### .vscode/

`c_cpp_properties`, `extensions`, `launch`

#### CMakeLists.txt/

`CMakeLists`, `simpleVoteIntrinsics`

#### README.md/

`README`

#### simpleVoteIntrinsics.cu/

`DAMAGES`, `MAX`, `VOTE_DATA_GROUP`, `checkCudaErrors`, `checkErrors1`, `checkErrors2`, `checkResultsVoteAllKernel2`, `checkResultsVoteAnyKernel1`, `checkResultsVoteAnyKernel3`, `cudaFree`, `cudaMalloc`, `cudaMemcpy`, `free`, `genVoteTestPattern`, `grid`, `gridBlock`, `kernel`, `main`, `printf`, `simpleVoteIntrinsics`, `thread`, `threadBlock`, `warp`

#### simpleVote_kernel.cuh/

`DAMAGES`, `Functions`, `SIMPLEVOTE_KERNEL_CU`, `VoteAllKernel2`, `VoteAnyKernel1`, `VoteAnyKernel3`, `__global__`, `kernel`, `simpleVote_kernel`, `thread`, `threads`, `vote`, `warp`


---

*Auto-generated subtree keyword index*
