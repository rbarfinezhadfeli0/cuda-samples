# Subtree Keyword Index for `Samples/4_CUDA_Libraries/nvJPEG_encoder/`

## Scope

This keyword index covers all files within `Samples/4_CUDA_Libraries/nvJPEG_encoder/` and all its subdirectories (recursive).

- **Files Indexed**: 22
- **Unique Keywords**: 25

## Keywords A-Z


### C

**CMakeLists**
- [Samples/4_CUDA_Libraries/nvJPEG_encoder/CMakeLists.txt](CMakeLists.txt_docs.md)

**c_cpp_properties**
- [Samples/4_CUDA_Libraries/nvJPEG_encoder/.vscode/c_cpp_properties.json](.vscode/c_cpp_properties.json_docs.md)

**checkCudaErrors**
- [Samples/4_CUDA_Libraries/nvJPEG_encoder/nvJPEG_encoder.cpp](nvJPEG_encoder.cpp_docs.md)

**cudaFree**
- [Samples/4_CUDA_Libraries/nvJPEG_encoder/nvJPEG_encoder.cpp](nvJPEG_encoder.cpp_docs.md)

**cudaMalloc**
- [Samples/4_CUDA_Libraries/nvJPEG_encoder/nvJPEG_encoder.cpp](nvJPEG_encoder.cpp_docs.md)


### D

**DAMAGES**
- [Samples/4_CUDA_Libraries/nvJPEG_encoder/nvJPEG_encoder.cpp](nvJPEG_encoder.cpp_docs.md)

**decodeEncodeOneImage**
- [Samples/4_CUDA_Libraries/nvJPEG_encoder/nvJPEG_encoder.cpp](nvJPEG_encoder.cpp_docs.md)

**dev_free**
- [Samples/4_CUDA_Libraries/nvJPEG_encoder/nvJPEG_encoder.cpp](nvJPEG_encoder.cpp_docs.md)

**dev_malloc**
- [Samples/4_CUDA_Libraries/nvJPEG_encoder/nvJPEG_encoder.cpp](nvJPEG_encoder.cpp_docs.md)


### E

**encode_params_t**
- [Samples/4_CUDA_Libraries/nvJPEG_encoder/nvJPEG_encoder.cpp](nvJPEG_encoder.cpp_docs.md)

**extensions**
- [Samples/4_CUDA_Libraries/nvJPEG_encoder/.vscode/extensions.json](.vscode/extensions.json_docs.md)


### F

**findParamIndex**
- [Samples/4_CUDA_Libraries/nvJPEG_encoder/nvJPEG_encoder.cpp](nvJPEG_encoder.cpp_docs.md)


### I

**is_interleaved**
- [Samples/4_CUDA_Libraries/nvJPEG_encoder/nvJPEG_encoder.cpp](nvJPEG_encoder.cpp_docs.md)


### L

**launch**
- [Samples/4_CUDA_Libraries/nvJPEG_encoder/.vscode/launch.json](.vscode/launch.json_docs.md)


### M

**main**
- [Samples/4_CUDA_Libraries/nvJPEG_encoder/nvJPEG_encoder.cpp](nvJPEG_encoder.cpp_docs.md)


### N

**nvJPEG_encoder**
- [Samples/4_CUDA_Libraries/nvJPEG_encoder/CMakeLists.txt](CMakeLists.txt_docs.md)
- [Samples/4_CUDA_Libraries/nvJPEG_encoder/nvJPEG_encoder.cpp](nvJPEG_encoder.cpp_docs.md)


### O

**oInputStream**
- [Samples/4_CUDA_Libraries/nvJPEG_encoder/nvJPEG_encoder.cpp](nvJPEG_encoder.cpp_docs.md)

**outputFile**
- [Samples/4_CUDA_Libraries/nvJPEG_encoder/nvJPEG_encoder.cpp](nvJPEG_encoder.cpp_docs.md)


### P

**processArgs**
- [Samples/4_CUDA_Libraries/nvJPEG_encoder/nvJPEG_encoder.cpp](nvJPEG_encoder.cpp_docs.md)


### R

**README**
- [Samples/4_CUDA_Libraries/nvJPEG_encoder/README.md](README.md_docs.md)


### S

**sFormat**
- [Samples/4_CUDA_Libraries/nvJPEG_encoder/nvJPEG_encoder.cpp](nvJPEG_encoder.cpp_docs.md)

**sInputPath**
- [Samples/4_CUDA_Libraries/nvJPEG_encoder/nvJPEG_encoder.cpp](nvJPEG_encoder.cpp_docs.md)

**sOutputPath**
- [Samples/4_CUDA_Libraries/nvJPEG_encoder/nvJPEG_encoder.cpp](nvJPEG_encoder.cpp_docs.md)

**sSubsampling**
- [Samples/4_CUDA_Libraries/nvJPEG_encoder/nvJPEG_encoder.cpp](nvJPEG_encoder.cpp_docs.md)

**sprintf**
- [Samples/4_CUDA_Libraries/nvJPEG_encoder/nvJPEG_encoder.cpp](nvJPEG_encoder.cpp_docs.md)



## Folder-Level Navigation

### Keywords by Subfolder

This section maps major keywords to the folders where they are most prevalent.


#### .vscode/

`c_cpp_properties`, `extensions`, `launch`

#### CMakeLists.txt/

`CMakeLists`, `nvJPEG_encoder`

#### README.md/

`README`

#### nvJPEG_encoder.cpp/

`DAMAGES`, `checkCudaErrors`, `cudaFree`, `cudaMalloc`, `decodeEncodeOneImage`, `dev_free`, `dev_malloc`, `encode_params_t`, `findParamIndex`, `is_interleaved`, `main`, `nvJPEG_encoder`, `oInputStream`, `outputFile`, `processArgs`, `sFormat`, `sInputPath`, `sOutputPath`, `sSubsampling`, `sprintf`


---

*Auto-generated subtree keyword index*
