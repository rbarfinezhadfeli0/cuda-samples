# Subtree Keyword Index for `Samples/4_CUDA_Libraries/simpleCUFFT_2d_MGPU/`

## Scope

This keyword index covers all files within `Samples/4_CUDA_Libraries/simpleCUFFT_2d_MGPU/` and all its subdirectories (recursive).

- **Files Indexed**: 6
- **Unique Keywords**: 23

## Keywords A-Z


### B

**block**
- [Samples/4_CUDA_Libraries/simpleCUFFT_2d_MGPU/simpleCUFFT_2d_MGPU.cu](simpleCUFFT_2d_MGPU.cu_docs.md)


### C

**CMakeLists**
- [Samples/4_CUDA_Libraries/simpleCUFFT_2d_MGPU/CMakeLists.txt](CMakeLists.txt_docs.md)

**c_cpp_properties**
- [Samples/4_CUDA_Libraries/simpleCUFFT_2d_MGPU/.vscode/c_cpp_properties.json](.vscode/c_cpp_properties.json_docs.md)

**cudaFree**
- [Samples/4_CUDA_Libraries/simpleCUFFT_2d_MGPU/simpleCUFFT_2d_MGPU.cu](simpleCUFFT_2d_MGPU.cu_docs.md)

**cudaMalloc**
- [Samples/4_CUDA_Libraries/simpleCUFFT_2d_MGPU/simpleCUFFT_2d_MGPU.cu](simpleCUFFT_2d_MGPU.cu_docs.md)

**cudaMemcpy**
- [Samples/4_CUDA_Libraries/simpleCUFFT_2d_MGPU/simpleCUFFT_2d_MGPU.cu](simpleCUFFT_2d_MGPU.cu_docs.md)


### D

**DAMAGES**
- [Samples/4_CUDA_Libraries/simpleCUFFT_2d_MGPU/simpleCUFFT_2d_MGPU.cu](simpleCUFFT_2d_MGPU.cu_docs.md)

**dimBlock**
- [Samples/4_CUDA_Libraries/simpleCUFFT_2d_MGPU/simpleCUFFT_2d_MGPU.cu](simpleCUFFT_2d_MGPU.cu_docs.md)

**dimGrid**
- [Samples/4_CUDA_Libraries/simpleCUFFT_2d_MGPU/simpleCUFFT_2d_MGPU.cu](simpleCUFFT_2d_MGPU.cu_docs.md)


### E

**extensions**
- [Samples/4_CUDA_Libraries/simpleCUFFT_2d_MGPU/.vscode/extensions.json](.vscode/extensions.json_docs.md)


### F

**for**
- [Samples/4_CUDA_Libraries/simpleCUFFT_2d_MGPU/simpleCUFFT_2d_MGPU.cu](simpleCUFFT_2d_MGPU.cu_docs.md)

**free**
- [Samples/4_CUDA_Libraries/simpleCUFFT_2d_MGPU/simpleCUFFT_2d_MGPU.cu](simpleCUFFT_2d_MGPU.cu_docs.md)


### G

**getLastCudaError**
- [Samples/4_CUDA_Libraries/simpleCUFFT_2d_MGPU/simpleCUFFT_2d_MGPU.cu](simpleCUFFT_2d_MGPU.cu_docs.md)


### K

**kernel**
- [Samples/4_CUDA_Libraries/simpleCUFFT_2d_MGPU/simpleCUFFT_2d_MGPU.cu](simpleCUFFT_2d_MGPU.cu_docs.md)


### L

**launch**
- [Samples/4_CUDA_Libraries/simpleCUFFT_2d_MGPU/.vscode/launch.json](.vscode/launch.json_docs.md)


### M

**main**
- [Samples/4_CUDA_Libraries/simpleCUFFT_2d_MGPU/simpleCUFFT_2d_MGPU.cu](simpleCUFFT_2d_MGPU.cu_docs.md)


### P

**printf**
- [Samples/4_CUDA_Libraries/simpleCUFFT_2d_MGPU/simpleCUFFT_2d_MGPU.cu](simpleCUFFT_2d_MGPU.cu_docs.md)


### R

**README**
- [Samples/4_CUDA_Libraries/simpleCUFFT_2d_MGPU/README.md](README.md_docs.md)


### S

**simpleCUFFT_2d_MGPU**
- [Samples/4_CUDA_Libraries/simpleCUFFT_2d_MGPU/CMakeLists.txt](CMakeLists.txt_docs.md)
- [Samples/4_CUDA_Libraries/simpleCUFFT_2d_MGPU/simpleCUFFT_2d_MGPU.cu](simpleCUFFT_2d_MGPU.cu_docs.md)

**solvePoisson**
- [Samples/4_CUDA_Libraries/simpleCUFFT_2d_MGPU/simpleCUFFT_2d_MGPU.cu](simpleCUFFT_2d_MGPU.cu_docs.md)

**solvePoissonEquation**
- [Samples/4_CUDA_Libraries/simpleCUFFT_2d_MGPU/simpleCUFFT_2d_MGPU.cu](simpleCUFFT_2d_MGPU.cu_docs.md)


### T

**thread**
- [Samples/4_CUDA_Libraries/simpleCUFFT_2d_MGPU/simpleCUFFT_2d_MGPU.cu](simpleCUFFT_2d_MGPU.cu_docs.md)



## Folder-Level Navigation

### Keywords by Subfolder

This section maps major keywords to the folders where they are most prevalent.


#### .vscode/

`c_cpp_properties`, `extensions`, `launch`

#### CMakeLists.txt/

`CMakeLists`, `simpleCUFFT_2d_MGPU`

#### README.md/

`README`

#### simpleCUFFT_2d_MGPU.cu/

`DAMAGES`, `__global__`, `block`, `cudaFree`, `cudaMalloc`, `cudaMemcpy`, `dimBlock`, `dimGrid`, `for`, `free`, `getLastCudaError`, `kernel`, `main`, `printf`, `simpleCUFFT_2d_MGPU`, `solvePoisson`, `solvePoissonEquation`, `thread`


---

*Auto-generated subtree keyword index*
