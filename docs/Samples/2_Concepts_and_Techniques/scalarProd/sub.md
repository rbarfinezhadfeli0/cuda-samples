# Subtree Keyword Index for `Samples/2_Concepts_and_Techniques/scalarProd/`

## Scope

This keyword index covers all files within `Samples/2_Concepts_and_Techniques/scalarProd/` and all its subdirectories (recursive).

- **Files Indexed**: 8
- **Unique Keywords**: 29

## Keywords A-Z


### A

**ACCUM_N**
- [Samples/2_Concepts_and_Techniques/scalarProd/scalarProd_kernel.cuh](scalarProd_kernel.cuh_docs.md)


### B

**block**
- [Samples/2_Concepts_and_Techniques/scalarProd/scalarProd_kernel.cuh](scalarProd_kernel.cuh_docs.md)


### C

**CMakeLists**
- [Samples/2_Concepts_and_Techniques/scalarProd/CMakeLists.txt](CMakeLists.txt_docs.md)

**c_cpp_properties**
- [Samples/2_Concepts_and_Techniques/scalarProd/.vscode/c_cpp_properties.json](.vscode/c_cpp_properties.json_docs.md)

**checkCudaErrors**
- [Samples/2_Concepts_and_Techniques/scalarProd/scalarProd.cu](scalarProd.cu_docs.md)

**cudaFree**
- [Samples/2_Concepts_and_Techniques/scalarProd/scalarProd.cu](scalarProd.cu_docs.md)

**cudaMalloc**
- [Samples/2_Concepts_and_Techniques/scalarProd/scalarProd.cu](scalarProd.cu_docs.md)

**cudaMemcpy**
- [Samples/2_Concepts_and_Techniques/scalarProd/scalarProd.cu](scalarProd.cu_docs.md)


### D

**DAMAGES**
- [Samples/2_Concepts_and_Techniques/scalarProd/scalarProd.cu](scalarProd.cu_docs.md)
- [Samples/2_Concepts_and_Techniques/scalarProd/scalarProd_cpu.cpp](scalarProd_cpu.cpp_docs.md)
- [Samples/2_Concepts_and_Techniques/scalarProd/scalarProd_kernel.cuh](scalarProd_kernel.cuh_docs.md)


### E

**extensions**
- [Samples/2_Concepts_and_Techniques/scalarProd/.vscode/extensions.json](.vscode/extensions.json_docs.md)


### F

**findCudaDevice**
- [Samples/2_Concepts_and_Techniques/scalarProd/scalarProd.cu](scalarProd.cu_docs.md)

**for**
- [Samples/2_Concepts_and_Techniques/scalarProd/scalarProd.cu](scalarProd.cu_docs.md)


### G

**grid**
- [Samples/2_Concepts_and_Techniques/scalarProd/scalarProd_kernel.cuh](scalarProd_kernel.cuh_docs.md)


### H

**hardware**
- [Samples/2_Concepts_and_Techniques/scalarProd/scalarProd_kernel.cuh](scalarProd_kernel.cuh_docs.md)


### I

**IMUL**
- [Samples/2_Concepts_and_Techniques/scalarProd/scalarProd_kernel.cuh](scalarProd_kernel.cuh_docs.md)


### K

**kernel**
- [Samples/2_Concepts_and_Techniques/scalarProd/scalarProd.cu](scalarProd.cu_docs.md)


### L

**launch**
- [Samples/2_Concepts_and_Techniques/scalarProd/.vscode/launch.json](.vscode/launch.json_docs.md)


### M

**main**
- [Samples/2_Concepts_and_Techniques/scalarProd/scalarProd.cu](scalarProd.cu_docs.md)


### R

**README**
- [Samples/2_Concepts_and_Techniques/scalarProd/README.md](README.md_docs.md)

**RandFloat**
- [Samples/2_Concepts_and_Techniques/scalarProd/scalarProd.cu](scalarProd.cu_docs.md)


### S

**scalarProd**
- [Samples/2_Concepts_and_Techniques/scalarProd/CMakeLists.txt](CMakeLists.txt_docs.md)
- [Samples/2_Concepts_and_Techniques/scalarProd/scalarProd.cu](scalarProd.cu_docs.md)

**scalarProdCPU**
- [Samples/2_Concepts_and_Techniques/scalarProd/scalarProd.cu](scalarProd.cu_docs.md)
- [Samples/2_Concepts_and_Techniques/scalarProd/scalarProd_cpu.cpp](scalarProd_cpu.cpp_docs.md)

**scalarProdGPU**
- [Samples/2_Concepts_and_Techniques/scalarProd/scalarProd_kernel.cuh](scalarProd_kernel.cuh_docs.md)

**scalarProd_cpu**
- [Samples/2_Concepts_and_Techniques/scalarProd/scalarProd_cpu.cpp](scalarProd_cpu.cpp_docs.md)

**scalarProd_kernel**
- [Samples/2_Concepts_and_Techniques/scalarProd/scalarProd_kernel.cuh](scalarProd_kernel.cuh_docs.md)


### T

**thread**
- [Samples/2_Concepts_and_Techniques/scalarProd/scalarProd_kernel.cuh](scalarProd_kernel.cuh_docs.md)


### W

**warp**
- [Samples/2_Concepts_and_Techniques/scalarProd/scalarProd.cu](scalarProd.cu_docs.md)
- [Samples/2_Concepts_and_Techniques/scalarProd/scalarProd_kernel.cuh](scalarProd_kernel.cuh_docs.md)



## Folder-Level Navigation

### Keywords by Subfolder

This section maps major keywords to the folders where they are most prevalent.


#### .vscode/

`c_cpp_properties`, `extensions`, `launch`

#### CMakeLists.txt/

`CMakeLists`, `scalarProd`

#### README.md/

`README`

#### scalarProd.cu/

`DAMAGES`, `RandFloat`, `checkCudaErrors`, `cudaFree`, `cudaMalloc`, `cudaMemcpy`, `findCudaDevice`, `for`, `kernel`, `main`, `scalarProd`, `scalarProdCPU`, `warp`

#### scalarProd_cpu.cpp/

`DAMAGES`, `scalarProdCPU`, `scalarProd_cpu`

#### scalarProd_kernel.cuh/

`ACCUM_N`, `DAMAGES`, `IMUL`, `__global__`, `__shared__`, `block`, `grid`, `hardware`, `scalarProdGPU`, `scalarProd_kernel`, `thread`, `warp`


---

*Auto-generated subtree keyword index*
