# Subtree Keyword Index for `Samples/3_CUDA_Features/memMapIPCDrv/`

## Scope

This keyword index covers all files within `Samples/3_CUDA_Features/memMapIPCDrv/` and all its subdirectories (recursive).

- **Files Indexed**: 7
- **Unique Keywords**: 39

## Keywords A-Z


### B

**barrierWait**
- [Samples/3_CUDA_Features/memMapIPCDrv/memMapIpc.cpp](memMapIpc.cpp_docs.md)

**block**
- [Samples/3_CUDA_Features/memMapIPCDrv/memMapIpc.cpp](memMapIpc.cpp_docs.md)
- [Samples/3_CUDA_Features/memMapIPCDrv/memMapIpc_kernel.cu](memMapIpc_kernel.cu_docs.md)


### C

**CMakeLists**
- [Samples/3_CUDA_Features/memMapIPCDrv/CMakeLists.txt](CMakeLists.txt_docs.md)

**c_cpp_properties**
- [Samples/3_CUDA_Features/memMapIPCDrv/.vscode/c_cpp_properties.json](.vscode/c_cpp_properties.json_docs.md)

**checkCudaErrors**
- [Samples/3_CUDA_Features/memMapIPCDrv/memMapIpc.cpp](memMapIpc.cpp_docs.md)

**childProcess**
- [Samples/3_CUDA_Features/memMapIPCDrv/memMapIpc.cpp](memMapIpc.cpp_docs.md)

**cpu_atomic_add32**
- [Samples/3_CUDA_Features/memMapIPCDrv/memMapIpc.cpp](memMapIpc.cpp_docs.md)

**cuMemCreate**
- [Samples/3_CUDA_Features/memMapIPCDrv/memMapIpc.cpp](memMapIpc.cpp_docs.md)


### D

**DAMAGES**
- [Samples/3_CUDA_Features/memMapIPCDrv/memMapIpc.cpp](memMapIpc.cpp_docs.md)
- [Samples/3_CUDA_Features/memMapIPCDrv/memMapIpc_kernel.cu](memMapIpc_kernel.cu_docs.md)

**DATA_BUF_SIZE**
- [Samples/3_CUDA_Features/memMapIPCDrv/memMapIpc.cpp](memMapIpc.cpp_docs.md)

**defined**
- [Samples/3_CUDA_Features/memMapIPCDrv/memMapIpc.cpp](memMapIpc.cpp_docs.md)


### E

**exec**
- [Samples/3_CUDA_Features/memMapIPCDrv/memMapIpc.cpp](memMapIpc.cpp_docs.md)

**extensions**
- [Samples/3_CUDA_Features/memMapIPCDrv/.vscode/extensions.json](.vscode/extensions.json_docs.md)


### F

**fault**
- [Samples/3_CUDA_Features/memMapIPCDrv/memMapIpc.cpp](memMapIpc.cpp_docs.md)

**file**
- [Samples/3_CUDA_Features/memMapIPCDrv/memMapIpc.cpp](memMapIpc.cpp_docs.md)

**findModulePath**
- [Samples/3_CUDA_Features/memMapIPCDrv/memMapIpc.cpp](memMapIpc.cpp_docs.md)

**for**
- [Samples/3_CUDA_Features/memMapIPCDrv/memMapIpc.cpp](memMapIpc.cpp_docs.md)

**fork**
- [Samples/3_CUDA_Features/memMapIPCDrv/memMapIpc.cpp](memMapIpc.cpp_docs.md)


### G

**getDefaultSecurityDescriptor**
- [Samples/3_CUDA_Features/memMapIPCDrv/memMapIpc.cpp](memMapIpc.cpp_docs.md)

**grid**
- [Samples/3_CUDA_Features/memMapIPCDrv/memMapIpc_kernel.cu](memMapIpc_kernel.cu_docs.md)


### K

**kernel**
- [Samples/3_CUDA_Features/memMapIPCDrv/memMapIpc.cpp](memMapIpc.cpp_docs.md)
- [Samples/3_CUDA_Features/memMapIPCDrv/memMapIpc_kernel.cu](memMapIpc_kernel.cu_docs.md)


### L

**launch**
- [Samples/3_CUDA_Features/memMapIPCDrv/.vscode/launch.json](.vscode/launch.json_docs.md)


### M

**MAX_DEVICES**
- [Samples/3_CUDA_Features/memMapIPCDrv/memMapIpc.cpp](memMapIpc.cpp_docs.md)

**main**
- [Samples/3_CUDA_Features/memMapIPCDrv/memMapIpc.cpp](memMapIpc.cpp_docs.md)

**memMapAllocateAndExportMemory**
- [Samples/3_CUDA_Features/memMapIPCDrv/memMapIpc.cpp](memMapIpc.cpp_docs.md)

**memMapGetDeviceFunction**
- [Samples/3_CUDA_Features/memMapIPCDrv/memMapIpc.cpp](memMapIpc.cpp_docs.md)

**memMapIPCDrv**
- [Samples/3_CUDA_Features/memMapIPCDrv/CMakeLists.txt](CMakeLists.txt_docs.md)

**memMapImportAndMapMemory**
- [Samples/3_CUDA_Features/memMapIPCDrv/memMapIpc.cpp](memMapIpc.cpp_docs.md)

**memMapIpc**
- [Samples/3_CUDA_Features/memMapIPCDrv/memMapIpc.cpp](memMapIpc.cpp_docs.md)

**memMapIpc_kernel**
- [Samples/3_CUDA_Features/memMapIPCDrv/memMapIpc_kernel.cu](memMapIpc_kernel.cu_docs.md)

**memMapUnmapAndFreeMemory**
- [Samples/3_CUDA_Features/memMapIPCDrv/memMapIpc.cpp](memMapIpc.cpp_docs.md)


### P

**PROCESSES_PER_DEVICE**
- [Samples/3_CUDA_Features/memMapIPCDrv/memMapIpc.cpp](memMapIpc.cpp_docs.md)

**PTX_FILE**
- [Samples/3_CUDA_Features/memMapIPCDrv/memMapIpc.cpp](memMapIpc.cpp_docs.md)

**parentProcess**
- [Samples/3_CUDA_Features/memMapIPCDrv/memMapIpc.cpp](memMapIpc.cpp_docs.md)


### R

**README**
- [Samples/3_CUDA_Features/memMapIPCDrv/README.md](README.md_docs.md)


### S

**shmStruct_st**
- [Samples/3_CUDA_Features/memMapIPCDrv/memMapIpc.cpp](memMapIpc.cpp_docs.md)

**strcat**
- [Samples/3_CUDA_Features/memMapIPCDrv/memMapIpc.cpp](memMapIpc.cpp_docs.md)


### T

**thread**
- [Samples/3_CUDA_Features/memMapIPCDrv/memMapIpc.cpp](memMapIpc.cpp_docs.md)
- [Samples/3_CUDA_Features/memMapIPCDrv/memMapIpc_kernel.cu](memMapIpc_kernel.cu_docs.md)



## Folder-Level Navigation

### Keywords by Subfolder

This section maps major keywords to the folders where they are most prevalent.


#### .vscode/

`c_cpp_properties`, `extensions`, `launch`

#### CMakeLists.txt/

`CMakeLists`, `memMapIPCDrv`

#### README.md/

`README`

#### memMapIpc.cpp/

`DAMAGES`, `DATA_BUF_SIZE`, `MAX_DEVICES`, `PROCESSES_PER_DEVICE`, `PTX_FILE`, `barrierWait`, `block`, `checkCudaErrors`, `childProcess`, `cpu_atomic_add32`, `cuMemCreate`, `defined`, `exec`, `fault`, `file`, `findModulePath`, `for`, `fork`, `getDefaultSecurityDescriptor`, `kernel`, `main`, `memMapAllocateAndExportMemory`, `memMapGetDeviceFunction`, `memMapImportAndMapMemory`, `memMapIpc`, `memMapUnmapAndFreeMemory`, `parentProcess`, `shmStruct_st`, `strcat`, `thread`

#### memMapIpc_kernel.cu/

`DAMAGES`, `__global__`, `block`, `grid`, `kernel`, `memMapIpc_kernel`, `thread`


---

*Auto-generated subtree keyword index*
