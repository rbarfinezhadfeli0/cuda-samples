# Subtree Keyword Index for `Samples/5_Domain_Specific/fastWalshTransform/`

## Scope

This keyword index covers all files within `Samples/5_Domain_Specific/fastWalshTransform/` and all its subdirectories (recursive).

- **Files Indexed**: 9
- **Unique Keywords**: 36

## Keywords A-Z


### B

**block**
- [Samples/5_Domain_Specific/fastWalshTransform/fastWalshTransform_kernel.cuh](fastWalshTransform_kernel.cuh_docs.md)


### C

**CMakeLists**
- [Samples/5_Domain_Specific/fastWalshTransform/CMakeLists.txt](CMakeLists.txt_docs.md)

**c_cpp_properties**
- [Samples/5_Domain_Specific/fastWalshTransform/.vscode/c_cpp_properties.json](.vscode/c_cpp_properties.json_docs.md)

**cudaFree**
- [Samples/5_Domain_Specific/fastWalshTransform/fastWalshTransform.cu](fastWalshTransform.cu_docs.md)

**cudaMalloc**
- [Samples/5_Domain_Specific/fastWalshTransform/fastWalshTransform.cu](fastWalshTransform.cu_docs.md)

**cudaMemcpy**
- [Samples/5_Domain_Specific/fastWalshTransform/fastWalshTransform.cu](fastWalshTransform.cu_docs.md)


### D

**DAMAGES**
- [Samples/5_Domain_Specific/fastWalshTransform/fastWalshTransform.cu](fastWalshTransform.cu_docs.md)
- [Samples/5_Domain_Specific/fastWalshTransform/fastWalshTransform_gold.cpp](fastWalshTransform_gold.cpp_docs.md)
- [Samples/5_Domain_Specific/fastWalshTransform/fastWalshTransform_kernel.cuh](fastWalshTransform_kernel.cuh_docs.md)

**dyadicConvolutionCPU**
- [Samples/5_Domain_Specific/fastWalshTransform/fastWalshTransform.cu](fastWalshTransform.cu_docs.md)
- [Samples/5_Domain_Specific/fastWalshTransform/fastWalshTransform_gold.cpp](fastWalshTransform_gold.cpp_docs.md)


### E

**ELEMENTARY_LOG2SIZE**
- [Samples/5_Domain_Specific/fastWalshTransform/fastWalshTransform_kernel.cuh](fastWalshTransform_kernel.cuh_docs.md)

**extensions**
- [Samples/5_Domain_Specific/fastWalshTransform/.vscode/extensions.json](.vscode/extensions.json_docs.md)


### F

**FWT**
- [Samples/5_Domain_Specific/fastWalshTransform/doc/FWT.doc](doc/FWT.doc_docs.md)

**FWT_KERNEL_CUH**
- [Samples/5_Domain_Specific/fastWalshTransform/fastWalshTransform_kernel.cuh](fastWalshTransform_kernel.cuh_docs.md)

**fastWalshTransform**
- [Samples/5_Domain_Specific/fastWalshTransform/CMakeLists.txt](CMakeLists.txt_docs.md)
- [Samples/5_Domain_Specific/fastWalshTransform/fastWalshTransform.cu](fastWalshTransform.cu_docs.md)

**fastWalshTransform_gold**
- [Samples/5_Domain_Specific/fastWalshTransform/fastWalshTransform_gold.cpp](fastWalshTransform_gold.cpp_docs.md)

**fastWalshTransform_kernel**
- [Samples/5_Domain_Specific/fastWalshTransform/fastWalshTransform_kernel.cuh](fastWalshTransform_kernel.cuh_docs.md)

**findCudaDevice**
- [Samples/5_Domain_Specific/fastWalshTransform/fastWalshTransform.cu](fastWalshTransform.cu_docs.md)

**for**
- [Samples/5_Domain_Specific/fastWalshTransform/fastWalshTransform_gold.cpp](fastWalshTransform_gold.cpp_docs.md)

**fwtBatch1Kernel**
- [Samples/5_Domain_Specific/fastWalshTransform/fastWalshTransform_kernel.cuh](fastWalshTransform_kernel.cuh_docs.md)

**fwtBatch2Kernel**
- [Samples/5_Domain_Specific/fastWalshTransform/fastWalshTransform_kernel.cuh](fastWalshTransform_kernel.cuh_docs.md)

**fwtBatchGPU**
- [Samples/5_Domain_Specific/fastWalshTransform/fastWalshTransform_kernel.cuh](fastWalshTransform_kernel.cuh_docs.md)

**fwtCPU**
- [Samples/5_Domain_Specific/fastWalshTransform/fastWalshTransform.cu](fastWalshTransform.cu_docs.md)
- [Samples/5_Domain_Specific/fastWalshTransform/fastWalshTransform_gold.cpp](fastWalshTransform_gold.cpp_docs.md)

**fwt_kernel_cuh**
- [Samples/5_Domain_Specific/fastWalshTransform/fastWalshTransform_kernel.cuh](fastWalshTransform_kernel.cuh_docs.md)


### G

**grid**
- [Samples/5_Domain_Specific/fastWalshTransform/fastWalshTransform_kernel.cuh](fastWalshTransform_kernel.cuh_docs.md)


### K

**kernel**
- [Samples/5_Domain_Specific/fastWalshTransform/fastWalshTransform.cu](fastWalshTransform.cu_docs.md)
- [Samples/5_Domain_Specific/fastWalshTransform/fastWalshTransform_gold.cpp](fastWalshTransform_gold.cpp_docs.md)
- [Samples/5_Domain_Specific/fastWalshTransform/fastWalshTransform_kernel.cuh](fastWalshTransform_kernel.cuh_docs.md)


### L

**launch**
- [Samples/5_Domain_Specific/fastWalshTransform/.vscode/launch.json](.vscode/launch.json_docs.md)


### M

**main**
- [Samples/5_Domain_Specific/fastWalshTransform/fastWalshTransform.cu](fastWalshTransform.cu_docs.md)

**modulateGPU**
- [Samples/5_Domain_Specific/fastWalshTransform/fastWalshTransform_kernel.cuh](fastWalshTransform_kernel.cuh_docs.md)

**modulateKernel**
- [Samples/5_Domain_Specific/fastWalshTransform/fastWalshTransform_kernel.cuh](fastWalshTransform_kernel.cuh_docs.md)


### O

**of**
- [Samples/5_Domain_Specific/fastWalshTransform/fastWalshTransform.cu](fastWalshTransform.cu_docs.md)


### P

**Podlozhnyuk**
- [Samples/5_Domain_Specific/fastWalshTransform/fastWalshTransform.cu](fastWalshTransform.cu_docs.md)

**patterns**
- [Samples/5_Domain_Specific/fastWalshTransform/fastWalshTransform_gold.cpp](fastWalshTransform_gold.cpp_docs.md)


### R

**README**
- [Samples/5_Domain_Specific/fastWalshTransform/README.md](README.md_docs.md)


### S

**slowWTcpu**
- [Samples/5_Domain_Specific/fastWalshTransform/fastWalshTransform.cu](fastWalshTransform.cu_docs.md)
- [Samples/5_Domain_Specific/fastWalshTransform/fastWalshTransform_gold.cpp](fastWalshTransform_gold.cpp_docs.md)


### T

**thread**
- [Samples/5_Domain_Specific/fastWalshTransform/fastWalshTransform_kernel.cuh](fastWalshTransform_kernel.cuh_docs.md)



## Folder-Level Navigation

### Keywords by Subfolder

This section maps major keywords to the folders where they are most prevalent.


#### .vscode/

`c_cpp_properties`, `extensions`, `launch`

#### CMakeLists.txt/

`CMakeLists`, `fastWalshTransform`

#### README.md/

`README`

#### doc/

`FWT`

#### fastWalshTransform.cu/

`DAMAGES`, `Podlozhnyuk`, `cudaFree`, `cudaMalloc`, `cudaMemcpy`, `dyadicConvolutionCPU`, `fastWalshTransform`, `findCudaDevice`, `fwtCPU`, `kernel`, `main`, `of`, `slowWTcpu`

#### fastWalshTransform_gold.cpp/

`DAMAGES`, `dyadicConvolutionCPU`, `fastWalshTransform_gold`, `for`, `fwtCPU`, `kernel`, `patterns`, `slowWTcpu`

#### fastWalshTransform_kernel.cuh/

`DAMAGES`, `ELEMENTARY_LOG2SIZE`, `FWT_KERNEL_CUH`, `__global__`, `__shared__`, `block`, `fastWalshTransform_kernel`, `fwtBatch1Kernel`, `fwtBatch2Kernel`, `fwtBatchGPU`, `fwt_kernel_cuh`, `grid`, `kernel`, `modulateGPU`, `modulateKernel`, `thread`


---

*Auto-generated subtree keyword index*
